...
author: filipe
image: 2024-07-01-supabase-realtime-broadcast-and-presence-authorization/thumb.png
...
This follows a similar approach to [Supabase Storage](https://supabase.com/docs/guides/storage/security/access-control) where we create a new table in the `realtime` schema that represents your [Channels](https://supabase.com/docs/guides/realtime/concepts#channels) as objects so you can write policies where SELECT and INSERT will determines what your users can access and use from Realtime.
...
- [Enable authorization on Channel subscription](#Enabling Authorization on Channel subscription)
...
To achieve it, we looked into our current solutions, namely how [Storage](https://supabase.com/docs/guides/storage/security/access-control) handles Access Control. Due to the nature of Realtime, our primitives are different as we have no assets stored in the database. So how did we achieve it?
...
The checks are done by running SELECT and INSERT queries on the new `realtime.messages` table which are then rollbacked so nothing is persisted. Then, based on the query result, we can determine the policies the user has for a given extension.

![The table that is used to test the policies.](../public/images/blog/2024-07-01-supabase-realtime-broadcast-and-presence-authorization/schema--dark.png)

The table that is used to test the policies.

![The table that is used to test the policies.](../public/images/blog/2024-07-01-supabase-realtime-broadcast-and-presence-authorization/schema--light.png)
...
![one-user-connection--dark.png](../public/images/blog/2024-07-01-supabase-realtime-broadcast-and-presence-authorization/one-user-connection--dark.png)

![one-user-connection--light.png](../public/images/blog/2024-07-01-supabase-realtime-broadcast-and-presence-authorization/one-user-connection--light.png)

Since we're doing this as soon as the user subscribe to Channel we should see a small increase in subscription but after that we do not run any queries on the database and we use the stored context to control the actions a user can perform.
...
- the policy cached in memory close to your users
- the policy cached for the duration of the connection or until the JWT expires
